// wap to add and delete a node from min heap 
#include<stdio.h>
#include<math.h>
...
void add_node_to_min_heap(int * a,int  key ,int n)
{
	a[n]=key;
	int i=n;
	while(i>=0)
...
void delete_node_from_min_heap(int *a,int key,int n)
{
	int i,pos;
	for(i=0;i<n;i++)
	{
...
int main()
{
	...
	n=6;
	int a[]={3,6,4,7,2,1};	
	int *b=create_min_heap(a,n);
	for(i=0;i<n;i++)
	{
		printf("%d ",b[i]);
	}
	add_node_to_min_heap(b,0,n);
	printf("\n");
	for(i=0;i<n+1;i++)
	{
		printf("%d ",b[i]);
	}
	delete_node_from_min_heap(b,0,n+1);
	printf("\n");
	for(i=0;i<n;i++)
	{
		printf("%d ",b[i]);
